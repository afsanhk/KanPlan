  ...
  const updateKanbanOrder = (projectID, statusIDs, kanbanOrders) => {
    ...
    statusIDs.forEach((statusID, index1) => {
      kanbanOrders[index1].forEach((taskID, index2) => {
        stateCopy.tasks[taskID].kanban_order = index2;
        stateCopy.tasks[taskID].status_id = Number(statusID);
        stateCopy.tasks[taskID].status = idToStatus[Number(statusID)];
      });
    });
